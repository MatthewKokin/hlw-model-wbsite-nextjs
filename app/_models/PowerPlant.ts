export default class PowerPlant {
    name: string;
    capacity: number; // in GW
    nuclearWaste: number = 0; // in metric tons
    generatedElectricity: number = 0;
    operationalDays: number = 0; // total days the plant has been operational
    daysBeforeClosing: number;
    isOperational: boolean;
    constructionFinishTime: number;
    isBuilding: boolean;
    type: string;
    uraniumYearlyConsumption: number = 0;
    wastePerDay: number = 0;
    ...
    continueBuilding(days: number) {
        this.constructionFinishTime -= days;
    }

    isOperatingOrBuilding(days: number) {
        if (this.isBuilding) {
            if (this.constructionFinishTime > days) {
                ...
                this.isBuilding = false;
                this.isOperational = true;
                this.operationalDays = days - this.constructionFinishTime; //
                this.continueBuilding(this.constructionFinishTime);
                return this.operate(this.operationalDays); // changed from operationalDays
            } else {
    ...
    prepareForBuilding(days: number) {
    ...
    generateElectricity(days: number) {
        this.generatedElectricity = (days / 365) * this.capacity;
    }

    generateWaste(days: number) {
        const wasteGenerated = this.capacity * this.wastePerDay * days;
        this.nuclearWaste += wasteGenerated;
        return wasteGenerated;
    }

    wasteMassToVolume(mass: number) {
        // Density is 18.4 tonne per m^3, assume 1:1 conditioning with glass, or if not conditioned, then the volume of a container
        return (mass / 18.4) * 1.5;
    }

    useUranium(days: number) {
